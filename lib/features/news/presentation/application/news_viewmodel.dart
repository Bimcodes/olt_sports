...
class NewsViewModel extends StateNotifier<NewsUIState> {
  ...
  late List<News> _allLoadedArticles;
  ...
  NewsViewModel(this._newsRepository) : super(const NewsUIStateInitial()) {
  ...
  Future<void> onRefresh() async {
    try {
      ...
      if (newsList.isEmpty) {
        state = const NewsUIStateEmpty(message: 'No news articles available.');
      } else {
        _allLoadedArticles = newsList;
        state = NewsUIStateSuccess(
          newsList: newsList,
